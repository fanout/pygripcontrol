...
class TestGripControl(unittest.TestCase):
    def test_create_hold(self):
        ...
        hold = json.loads(
            create_hold(
                "mode", "channel", Response("code", "reason", "headers", "body")
            )
        )
        ...
        hold = json.loads(
            create_hold(
                "mode",
                "channel",
                Response("code", "reason", "headers", pack("hhh", 253, 254, 255)),
            )
        )
        self.assertEqual(hold["hold"]["mode"], "mode")
        self.assertEqual(
                ...
                json.dumps(
                    {
                        ...
                        "reason": "reason",
                        "headers": "headers",
                        "body-bin": b64encode(pack("hhh", 253, 254, 255)).decode(
                            "utf-8"
                        ),
                    }
                )
    ...
    def test_parse_grip_uri(self):
        ...
        self.assertEqual(config["key"], b64decode("geag+21321=="))
        uri = (
            "http://api.fanout.io/realm/realm?iss=realm"
        ...
        try:
            ...
            self.assertEqual(
                config["control_uri"],
                "http://api.fanout.io/realm/realm?" + "param2=value2&param1=value1",
            )
        self.assertEqual(config["control_iss"], "realm")
        self.assertEqual(config["key"], b64decode("geag121321=="))
    ...
    def test_validate_sig(self):
        token = jwt.encode(
            {
                "iss": "realm",
                "exp": calendar.timegm(datetime.utcnow().utctimetuple()) + 3600,
            },
            "key",
        )
    ...
    def test_create_grip_channel_header(self):
        ...
        self.assertEqual(header, "channel")
        header = create_grip_channel_header(Channel("channel", "prev-id"))
        self.assertEqual(header, "channel; prev-id=prev-id")
        header = create_grip_channel_header(
            [Channel("channel1", "prev-id1"), Channel("channel2", "prev-id2")]
        )
        self.assertEqual(
            header, "channel1; prev-id=prev-id1, channel2; prev-id=prev-id2"
        )

    def test_create_hold_response(self):
        hold = json.loads(
            create_hold_response(
                "channel", Response("code", "reason", "headers", "body")
            )
        )
    ...
    def test_create_hold_stream(self):
        hold = json.loads(
            create_hold_stream("channel", Response("code", "reason", "headers", "body"))
        )
    ...
    def test_decode_websocket_events(self):
        if is_python3:
            events = decode_websocket_events(
                b"OPEN\r\nTEXT 5\r\nHello"
                + b"\r\nTEXT 0\r\n\r\nCLOSE\r\nTEXT\r\nCLOSE\r\n"
            )
        else:
    ...
    def test_websocket_control_message(self):
        ...
        message = json.loads(
            websocket_control_message("type", {"arg1": "val1", "arg2": "val2"})
        )
        self.assertEqual(message["type"], "type")
    ...
    def test_parse_channels(self):
        ...
        channels = _parse_channels(
            [Channel("channel1", "prev-id"), Channel("channel2")]
        )
    ...
    def test_get_hold_channels(self):
        ...
        hold_channels = _get_hold_channels([Channel("channel", "prev-id")])
        self.assertEqual(hold_channels[0], {"name": "channel", "prev-id": "prev-id"})
        hold_channels = _get_hold_channels(
            [Channel("channel1", "prev-id1"), Channel("channel2", "prev-id2")]
        )
        self.assertEqual(hold_channels[0], {"name": "channel1", "prev-id": "prev-id1"})
        self.assertEqual(hold_channels[1], {"name": "channel2", "prev-id": "prev-id2"})
    ...
    def test_get_hold_response(self):
        ...
        self.assertEqual(
            response["body-bin"], b64encode(pack("hhh", 253, 254, 255)).decode("utf-8")
        )
        response = _get_hold_response(
            Response("code", "reason", {"header1": "val1"}, "body\u2713")
        )
    ...
    def test_is_unicode_instance(self):
        self.assertTrue(_is_unicode_instance("hello"))
    ...
    def test_bin_or_text(self):
        ...
        self.assertEqual(
            _bin_or_text(pack("hhh", 253, 254, 255)),
            (False, pack("hhh", 253, 254, 255)),
        )

    def test_timestamp_utcnow(self):
        self.assertEqual(
            _timestamp_utcnow(), calendar.timegm(datetime.utcnow().utctimetuple())
        )
